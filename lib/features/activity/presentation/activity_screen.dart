...
import '../../../features/step_tracker/providers/step_tracker_providers.dart';
...
class _ActivityScreenState extends ConsumerState<ActivityScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _requestStepTrackerPermissions();
    });
  }

  Future<void> _requestStepTrackerPermissions() async {
    if (_permissionRequested) return;
    _permissionRequested = true;

    try {
      final notifier = ref.read(stepTrackerNotifierProvider.notifier);
      await notifier.startTracking();
    } catch (e) {
      print('만보기 권한 요청 실패: $e');
    }
  }
  ...
  Future<void> _refreshStepData() async {
    try {
      // Step Tracker 새로고침
      final stepNotifier = ref.read(stepTrackerNotifierProvider.notifier);
      await stepNotifier.refreshSteps();

      // 새로고침 프로바이더 갱신
      ref.read(stepRefreshProvider.notifier).update((state) => state + 1);

      // 사용자에게 피드백
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('걸음 수 데이터를 새로고침했습니다'),
