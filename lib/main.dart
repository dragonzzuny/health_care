...
void main() async {
  ...
  // 환경 변수나 안전한 저장소에서 API 키를 자동으로 로드
  // (.env 파일, secure storage, 플랫폼별 설정 등)
  await AppConfig.initializeFromEnvironment();

  // 또는 API 키 없이 ML Kit만 사용하려면:
  // await AppConfig.initializeWithoutApiKey();
...
final dataInitializationProvider = FutureProvider<bool>((ref) async {
  ...
  try {
    ...
    if (!isInitialized) {
      ...
      developer.log('✅ 앱 시작 시 데이터 초기화 완료');
    } else {
      developer.log('✅ 데이터가 이미 초기화되어 있습니다');
    }
    
    return true;
  } catch (e) {
    developer.log('❌ 데이터 초기화 실패: $e');
    return false;
  }
});
